  ...
  return (
    <header className="bg-white dark:bg-[#1E1E1E] backdrop-blur-md border-b border-gray-200 dark:border-gray-800 px-6 h-[68px] flex justify-between items-center sticky top-0 z-40 transition-all duration-300 shadow-sm relative flex-shrink-0">
      ...
      <div className="flex items-center gap-4">
        <button onClick={onMenuClick} className="lg:hidden p-2 -ml-2 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" aria-label="Open menu">
          <FiMenu className="w-5 h-5" />
        </button>
        <button onClick={onToggleSidebar} className="hidden lg:flex p-2 -ml-2 rounded-lg text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors" title={sidebarCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}>
          {sidebarCollapsed ? <FiChevronsRight className="w-5 h-5" /> : <FiChevronsLeft className="w-5 h-5" />}
        </button>
        <div className="flex lg:hidden items-center gap-2.5">
          {logoUrl && <img src={logoUrl} alt="System logo" className="h-8 w-8 rounded-xl object-contain bg-white/60 dark:bg-zinc-900/30 border border-gray-200 dark:border-gray-800 p-1" />}
          <h1 className="text-xl font-bold text-gray-800 dark:text-zinc-100 tracking-tight">CCS Profiling</h1>
        </div>
      </div>
      ...
      <div className="hidden md:flex items-center gap-3 px-4 py-1.5 bg-gray-50/50 dark:bg-white/[0.03] border border-gray-100 dark:border-gray-800/50 rounded-full shadow-inner-sm group hover:border-brand-500/20 transition-all duration-300">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold text-gray-600 dark:text-zinc-400 whitespace-nowrap uppercase tracking-widest group-hover:text-brand-500 transition-colors">{semester || 'N/A'}</span>
        </div>
        <div className="w-px h-3 bg-gray-200 dark:bg-gray-800" />
        <span className="text-[10px] font-black text-gray-400 dark:text-zinc-500 tracking-[0.12em] whitespace-nowrap uppercase group-hover:text-gray-600 dark:group-hover:text-zinc-300 transition-colors">AY {academicYear || 'N/A'}</span>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative pl-1 sm:pl-0" ref={dropdownRef}>
          <Button variant="primary" size="icon" className="w-[30px] h-[30px] rounded-full shadow-sm ring-1 ring-[#FF6B00] ring-offset-1 ring-offset-white dark:ring-offset-gray-900 shadow-lg shadow-[#FF6B00]/20" onClick={() => setDropdownOpen(!dropdownOpen)}>
            {profilePicture ? (
              <img src={profilePicture} alt={`${user?.firstname} ${user?.lastname}`} className="w-full h-full object-cover rounded-full" />
            ) : (
          ...
          {dropdownOpen && (
            <div className="absolute right-0 mt-3 w-56 bg-white/95 dark:bg-[#1E1E1E]/95 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)] border border-gray-100 dark:border-gray-800 p-1 z-50 overflow-hidden rounded-2xl origin-top-right transition-all animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="px-4 py-3.5 mb-1 border-b border-gray-100 dark:border-gray-800/50 flex items-center gap-3.5">
                {profilePicture ? (
                  <img src={profilePicture} alt="Profile" className="w-9 h-9 rounded-xl object-cover ring-2 ring-gray-50 dark:ring-white/5" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] font-black text-[#FF6B00] uppercase tracking-[0.15em] opacity-90">{user?.role || 'User'}</p>
                </div>
              </div>

              {/* Menu Items */}
              <div className="space-y-0.5">
                <button onClick={() => { setDropdownOpen(false); navigate('/profile'); }} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-[#FF6B00] dark:hover:text-[#FF6B00] hover:bg-gray-50 dark:hover:bg-white/5 rounded-xl transition-all duration-200 group">
                ...
                <button onClick={() => { setDropdownOpen(false); navigate('/settings'); }} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-[#FF6B00] dark:hover:text-[#FF6B00] hover:bg-gray-50 dark:hover:bg-white/5 rounded-xl transition-all duration-200 group">
                  <div className="w-7 h-7 rounded-lg bg-gray-50 dark:bg-white/5 flex items-center justify-center group-hover:bg-[#FF6B00]/10 transition-colors">
                ...
                <button onClick={handleLogout} disabled={isProcessing} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-xl transition-all duration-200 group disabled:opacity-50">
                  <div className="w-7 h-7 rounded-lg bg-red-50 dark:bg-red-500/10 flex items-center justify-center group-hover:bg-red-500/20 transition-colors">
                    {isProcessing ? <div className="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" /> : <FiLogOut className="w-3.5 h-3.5" />}
                  </div>
