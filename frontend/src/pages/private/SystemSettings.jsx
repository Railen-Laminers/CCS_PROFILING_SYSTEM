...
const SystemSettings = () => {
  ...
  const handleThemeChange = (e) => {
    ...
    if (selectedTheme === 'light' && isDark) {
      toggleTheme();
    } else if (selectedTheme === 'dark' && !isDark) {
      toggleTheme();
    }
  };
  ...
  // Restrict access: only admin users can see the settings
  if (!user || user.role !== 'admin') {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">You do not have permission to view system settings.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-6">
        ...
        {/* Backup System Now button removed */}
      </div>
                ...
                {notificationSettings.map((setting) => (
                  <div key={setting.id} className="p-8 flex items-center justify-between hover:bg-gray-50/30 dark:hover:bg-[#252525]/30 transition-all group">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-12 items-center rounded-full transition-all focus:outline-none ring-offset-2 focus:ring-2 focus:ring-brand-500/50 ${setting.enabled ? 'bg-brand-500 shadow-inner' : 'bg-gray-200 dark:bg-zinc-800'
                        }`}
                    >
                      <span
                        className={`${setting.enabled ? 'translate-x-7' : 'translate-x-1'
          ...
          <div className="space-y-10">
            <Card className="bg-white dark:bg-[#1E1E1E] border border-gray-200 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <CardContent className="p-8 space-y-10">
                {/* Theme Mode Selection - Select Dropdown */}
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                  <div className="md:col-span-1">
                    <h4 className="text-[15px] font-bold text-gray-900 dark:text-zinc-100 tracking-tight">Theme Preference</h4>
                    <p className="text-[13px] text-gray-500 dark:text-zinc-500 mt-1">Choose between light and dark interface appearance.</p>
                  </div>
