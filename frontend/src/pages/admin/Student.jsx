    ...
    const handleCreate = async (e) => {
        ...
        try {
            const studentData = {
                ...
                sports_activities: formData.sports_activities ? formData.sports_activities.split(',').map(s => s.trim()).filter(Boolean) : [],
                organizations: formData.organizations ? formData.organizations.split(',').map(s => s.trim()).filter(Boolean) : [],
                behavior_discipline_records: formData.behavior_discipline_records ? formData.behavior_discipline_records.split(',').map(s => s.trim()).filter(Boolean) : [],
                current_subjects: formData.current_subjects ? formData.current_subjects.split(',').map(s => s.trim()).filter(Boolean) : [],
                academic_awards: formData.academic_awards ? formData.academic_awards.split(',').map(s => s.trim()).filter(Boolean) : [],
                events_participated: formData.events_participated ? formData.events_participated.split(',').map(s => s.trim()).filter(Boolean) : [],
            };
