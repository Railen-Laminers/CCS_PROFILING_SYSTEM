...
class EmailService {
   ...
   */
  static async send2FAEmail(email, otp) {
    const transporter = this.getTransporter();
      ...
      html: `
        <div style="font-family: 'Inter', system-ui, -apple-system, sans-serif; background-color: #f9fafb; padding: 40px 20px; line-height: 1.6;">
          <div style="max-width: 500px; margin: 0 auto; background: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
            ...
            <div style="padding: 40px 32px;">
              <h2 style="color: #111827; margin: 0 0 16px; font-size: 20px; font-weight: 600;">Verification Code</h2>
              <p style="color: #4b5563; margin-bottom: 24px;">Your security is our priority. Please use the following code to complete your sign-in process.</p>
              
              <div style="text-align: center; margin: 32px 0;">
                <div style="display: inline-block; padding: 16px 40px; background-color: #f3f4f6; color: #111827; border-radius: 12px; font-weight: 800; font-size: 32px; letter-spacing: 0.25em; border: 1px solid #e5e7eb;">
                  ${otp}
                </div>
              </div>
              
              <p style="color: #6b7280; font-size: 14px; margin-top: 24px;">This code will expire in 10 minutes. If you didn't request this code, please secure your account immediately.</p>
            </div>
            <div style="background-color: #f3f4f6; padding: 24px 32px; text-align: center;">
              <p style="color: #9ca3af; font-size: 12px; margin: 0;">&copy; ${new Date().getFullYear()} Profiling System. All rights reserved.</p>
            </div>
